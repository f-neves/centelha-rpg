...
//
// ---------------------------------------------------------------------------------------
// DOIS SISTEMAS, UM CONJUNTO DE REGRAS (§15 do Combate_Tempo.md)
//
// O jogo terá dois sistemas de tempo, à escolha do mestre, e as MESMAS regras nos dois:
//   · o **normal**, o de hoje: a ação custa a Velocidade e resolve no primeiro Tick
//     (`usarPreparo: false`, e todo Preparo vira 0);
//   · o **P/G/R**: a mesma Velocidade partida em Preparo, Golpe e Recuperação.
// Tudo o que foi decidido (a escada de penalidades, a rajada, a dupla, a dívida de Ticks) é
// escrito em moeda comum — Ticks, dados e pontos de Defesa — e vale nos dois. Os presets
// prontos são REGRAS_HOJE, REGRAS_NORMAL, REGRAS_PGR e REGRAS_K1.
//
// A ESCADA (§14.11). Estar comprometido com uma ação custa Defesa, e a moeda é o −2 da Guarda
// sob pressão do capítulo IX:
//   · Preparo −2 · Golpe −4 (o dobro) · Recuperação −2 POR GOLPE DADO.
// Mais os −2 por ataque recebido, que acumulam sem teto e só zeram quando o ciclo fecha.
//
// A AGENDA (`offs`). Em que Ticks os golpes caem, contados da declaração. Ela serve aos três
// casos com o mesmo código: golpe simples `[P]`, empunhadura dupla `[P, P+1]` e rajada
// `[P, P+1, P+2]` (§14.12 e §14.13).
//
// ---------------------------------------------------------------------------------------
...
// e o K25 (a Defesa da arma e a do escudo somam na ficha).
...
export const REGRAS_PADRAO = {
  ...
  centelhaMult: 1,            // a Centelha soma +1 no ataque e nas defesas (K23; era 2 até 20/08)
  ...
  penDadosCadeia: [0, 1, 2, 3], // rajada: dados perdidos por golpe da sequência (o 1º é cheio)
  modFixoDupla: [0, 0],       // modificador fixo de rolagem por mão (a Ambidestria mexe aqui)
  empateAcerta: false,        // true = igualar a Defesa ja acerta (a convencao >=)
  escalonar: true,            // as penalidades crescem a cada repeticao da cadeia
  alivioOffDV: 2,             // DV devolvida no Tick do Golpe por ter algo na outra mão
...
/** A régua P/G/R por classe: em que Tick o golpe cai, contado da declaração. */
export const PREPARO_PGR = {
...
/**
 * As regras decididas em 20/08/2026, na forma que vale nos DOIS sistemas: a escada de
 * penalidades, a rajada e a empunhadura dupla. É a base dos presets abaixo.
 */
const DECIDIDO_20_08 = {
  preparoDV: 2, golpeDV: 4, recupDV: 2,   // a escada (§14.11)
  // A escada É o custo de atacar, então a regra antiga ("o golpe que você dá soma −2 na sua
  // guarda") sai: senão o mesmo golpe é cobrado duas vezes. E a guarda se refaz quando o ciclo
  // fecha, não no golpe (no sistema normal os dois momentos são o mesmo Tick).
  atacarCustaGuarda: false, guardaEm: 'declara',
  alivioOffDV: 2,                          // segurar sem golpear alivia o Tick do Golpe
  pressaoSegundaMao: 0,                    // a Recuperação por golpe dado já cobra o 2º golpe
  penDadosDupla: [1, 1],                   // dupla: −1d6 nas duas mãos (§14.13)
  penDadosCadeia: [0, 1, 2, 3],            // rajada: −1d6 acumulativo (§14.12)
  escalonar: true,
};

/** O SISTEMA NORMAL como está hoje no site, sem nenhuma regra nova. A referência. */
export const REGRAS_HOJE = comRegras(REGRAS_PADRAO, {
  usarPreparo: false, redirecionar: false,
  fora: { ...REGRAS_PADRAO.fora, ligada: false }, interrupcao: 0,
});

/** O SISTEMA NORMAL com as regras de 20/08 (escada, rajada, dupla, dívida de Ticks). */
export const REGRAS_NORMAL = comRegras(REGRAS_PADRAO, { usarPreparo: false, ...DECIDIDO_20_08 });

/** O SISTEMA P/G/R com as mesmas regras. */
...
/** A régua de 18/08 (o K1), guardada para reproduzir as tabelas das §2 a §9. */
export const REGRAS_K1 = REGRAS_PADRAO;
...
export function lutador(spec, cat) {
  ...
  const agenda = (() => {
    ...
    const rx = spec.rExtra || 0;
    const t2 = spec.extraDaR === false ? ultimo + 1 + Rbase + rx : Math.max(total, ultimo + 1) + rx;
    return { offs, total: t2 };
  })();

  return {
    // ficha
    nome: spec.rotulo || w.nome, regrasCentelha: R.centelhaMult ?? 1, off: spec.off || (spec.dupla ? 'arma' : null), escudo: spec.escudo || null, arma: w, armaOff: spec.armaOff ? (cat.armas[spec.armaOff] || null) : null, armadura: a, usarDefArma: !!R.usarDefesaArma,
    ...
    // A dupla e a rajada compartilham a geometria (Ticks de Golpe seguidos) e diferem na régua:
    // a dupla é −1d6 nas duas mãos, a rajada é o primeiro golpe cheio e −1d6 acumulativo.
    tipo: cad > 1 ? 'cadeia' : (spec.rajada ? 'rajada' : (golpesN > 1 ? 'dupla' : 'simples')),
    porRep: cad > 1 ? 1 : Math.max(1, agenda.offs.length),
    nGolpes: agenda.offs.length * porTempo,
...
export const defesaBase = (c) => c.ah * 2 + c.centelha * (c.regrasCentelha ?? 1) - c.armadura.penalidade
...
export function atacar(A, D, R, rnd, log = null) {
  ...
  // A penalidade de estar atacando. Escalona com a REPETICAO (a cadeia prepara de novo a cada
  // golpe) e a Recuperacao escalona com quantos golpes foram dados.
  let comprom = 0;
  if (D.emAcao) {
    const tAgora = D.tickAgora;
    if (D.pend) {
      const jaDados = D.pend.offs.filter((x) => x < tAgora).length;
      const k = R.escalonar ? Math.floor(jaDados / D.porRep) + 1 : 1;
      const pk = (R.preparoDV ?? 0) * k;
      if (!D.emGolpe) comprom = pk;
      else {
        // A outra mao continua guardando enquanto a arma principal esta comprometida.
        const alivio = (D.off && D.tipo !== 'dupla') ? (R.alivioOffDV || 0) : 0;
        comprom = Math.max(pk, (R.golpeDV ?? 0) * k - alivio);
      }
    } else {
      comprom = (R.recupDV ?? 0) * (R.escalonar ? (D.nUltimo || 1) : 1);
    }
  }
  let efDef = defesaBase(D) + (D.escudo ? D.escudo.bloq : 0) - R.pressao * D.guard - comprom;
  ...
  total += (A.ah % 2 ? 2 : 0) + A.arma.acerto + A.centelha * (R.centelhaMult ?? 1)
    - A.armadura.penalidade - A.arma.pen
    - (A.penalidadeAgora || 0) + (A.modFixo || 0) - (A.escudo ? A.escudo.pen : 0)
  ...
  if (total > efDef - (R.empateAcerta ? 1 : 0)) {
    acertou = true;
...
export function cena(timeA, timeB, R, rnd, { narrar = false, tetoTicks = 4000 } = {}) {
  ...
  for (let t = 1; t <= tetoTicks; t++) {
    ...
    for (const c of todos) {
      ...
      c.emAcao = true; c.nUltimo = c.nGolpes;
    ...
    for (const c of prontos) {
      ...
      for (let m = inicio; m < inicio + porTick; m++) {
        c.maoAtual = m;
        const regua = c.tipo === 'dupla' ? R.penDadosDupla : R.penDadosCadeia;
        c.penDados = c.totalGolpes > 1 ? regua[Math.min(m, regua.length - 1)] : 0;
        c.modFixo = (c.tipo === 'dupla' && R.modFixoDupla) ? (R.modFixoDupla[Math.min(m, R.modFixoDupla.length - 1)] || 0) : 0;
        const armaDaVez = c.arma;
        if (c.tipo === 'dupla' && m > 0 && c.armaOff) c.arma = c.armaOff;
        if (log) log(`[T${t}] sai o golpe de ${c.nome}${c.golpes > 1 ? ` (mão ${m === 0 ? 'hábil' : 'inábil'})` : ''}.`);
        atacar(c, vitima, R, rnd, log);
        c.arma = armaDaVez;
      }
      c.penDados = 0; c.modFixo = 0; c.maoAtual = 0;
    }
