...
- [ ] **K25 · [DECIDIR] A Defesa da arma e a do escudo somam, e o escudeiro vira parede.** A
  ...
  (`ficha-engine.ts:1540`, `Bloqueio soma a Defesa das armas/escudos do conjunto EM USO`). Com
...
- [ ] **K31 · [FAZER] O Grid anda a Corrida só na primeira fase: a regra publicada tem duas, e o código aplica metade.** Aberto pelo Arquiteto em 24/09/2026, por decisão do humano, **com a direção escrita e sem abrir o conserto** (a rodada 101 está em curso). **A regra, publicada desde 11/06/2026 (`1418de1`):** `combate.md:298-302` e a nota de `regras.json` `derivados.deslocamento`: os 3 primeiros Ticks de uma Corrida vão na velocidade de ARRANQUE (`2 + Força ÷ 4 + Atletismo ÷ 4 + Destreza ÷ 2`), e do 4º em diante na de CORRIDA (`4 + 0,75 Destreza + 0,5 Atletismo`). **O que o Grid faz desde `6ba575e` (28/08/2026):** `passoNoModo` (`src/pages/mesa/grid.astro`) anda o Arranque em TODOS os Ticks de uma Corrida e de uma Investida, e nunca passa à segunda fase; o corte está justificado só num comentário ("quase toda perseguição de combate cabe neles"), e ninguém o decidiu. Forma no `CATALOGO.md`: "a metade de uma regra publicada que o código largou num comentário".

  **A DIREÇÃO, decidida pelo humano em 24/09/2026 (a opção 1):** o Grid passa a seguir as duas fases. **E o "50 a 67%" de `combate.md:291` e de `regras.json:2477` FICA:** ele é a faixa do Arranque (entrou em `0f191fe`, 22/08, com o exemplo do Kael, cujo "corre 6" é o arranque dele). O conserto do texto é dizer que aquela faixa é a da PRIMEIRA fase, e não a da Corrida inteira, senão o capítulo continua ensinando metade da própria regra.

  **POR QUE ISTO É CONSERTO E NÃO DECISÃO, nas palavras do humano:** a pergunta nasceu como decisão (a raiz 6 da lista única, 24/09/2026) porque ele não sabia que a regra estava publicada. As outras duas opções (a Corrida de combate ser só o arranque, reescrevendo a regra; ou a regra ficar e o Grid seguir no arranque como simplificação declarada) reescreviam regra publicada para justificar implementação incompleta, e isso é o contrário da ordem das coisas aqui: o dado e o capítulo definem, o código segue.

  **O argumento que a medição derrubou, e ele veio do humano:** "o personagem fraco corre e não sai do lugar". Medido pelo Arquiteto: com Destreza 1 e Atletismo 0, ele anda 2 m por Tick em Batalha e 3 no Arranque (4 com Força 4 ou mais), e a Corrida de topo dele seria 5. O Arranque igual ao Batalha acontece em 8 dos 252 perfis (Destreza 1 a 6, Atletismo 0 a 6, Força 1 a 6), todos por arredondamento, com Destreza 1 ou 2 e Força 1 ou 2: `D1 A1 F1`, `D1 A1 F2`, `D1 A2 F1`, `D1 A5 F1`, `D1 A5 F2`, `D1 A6 F1`, `D2 A0 F1`, `D2 A4 F1`. Sem armadura e sem a fração de raça.

  **O número que importa depois do conserto, e ele é pergunta de equilíbrio do humano:** em **111 dos 252** perfis o Arranque fica abaixo de +50% sobre o Batalha. Ou seja, nos três primeiros Ticks, quase metade dos perfis corre menos do que a faixa publicada promete, e é justamente a fase que quase toda perseguição de combate usa. Não é deste item decidir; ele só não pode ser esquecido quando o conserto sair.
