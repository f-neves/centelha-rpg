...
**Nota de procedência (07/09/2026):** este arquivo era `.claude/CLAUDE.local.md` no
worktree `centelha-revisora`, excluído pelo `.git/info/exclude` e por isso existia
só naquele disco. Copiado aqui, byte a byte, quando as duas instâncias antigas
(executora e revisora) fecharam, para não virar decisão importante fora do
repositório. O conteúdo abaixo é o texto original, sem edição: onde ele fala de
"você" e "esta instância", fala da revisora antiga, e é registro histórico, não
instrução ativa. O que dele vale como método para a próxima revisora é decisão de
quem ler.
...
0. **Antes de qualquer coisa, confirme que você está no worktree certo, e que este
   ...
   **E "achei o contrato" não é "abri no lugar certo".** Em 04/09/2026 esta
   ...
   protege antes de cada checkout (`scripts/duo.mjs:55` · `vigilâncias`), e ela chega igual pelo
...
1. **O critério de aceitação do conserto.** Quando o commit é conserto e não
   ...
   **E uma sexta, que vale para TODO diff e é a mais barata de todas:**
   ...
   - `gen-monsters.mjs:207` · `CAMPOS_MESA` · afirma GARANTIA, em letras: "esquecer disso aparece
