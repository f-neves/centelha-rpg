...
| **V15** | **ancestralidade.** Todo evento `dano` tem um `decl` ancestral na mesma batalha, pelo campo `aid`, e todo `aid` de `golpe.resolve` existe em algum `decl` anterior | fim do passo 5, e de novo no fim da batalha sobre o log inteiro | **não se aplica ao Grid**: o `aid` nasce com o harness. Ele existe porque o campo tem de sobreviver à re-projeção, que reescreve a agenda, e um `aid` perdido ali produz **tempo morto menor e perfeitamente crível**. É o invariante que guarda a métrica que a §D8b elegeu como um dos dois critérios de reprovação |
