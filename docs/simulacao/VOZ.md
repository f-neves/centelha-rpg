...
- **Desfazer não é a rede que este documento supunha.** Cobre posição e Vida
  (`src/pages/mesa/grid.astro:11205` · `async function desfazer`), não cobre Mana, a declaração do golpe, o Tick nem a
...
- **Mana não distingue dar de tirar.** É uma função só (`src/pages/mesa/grid.astro:10898` · `async function ajustarMana`), então
...
`src/pages/mesa/grid.astro:9974` · `O CAMPO GUARDA AS FACES, E NÃO O TOTAL`. Quem soma é a folha,
...
`src/pages/mesa/grid.astro:10158` · `const CAMPOS_ATQ`, e
`src/pages/mesa/grid.astro:10171` · `const CAMPOS_ALVO`.
...
|---|---|---|
| **campo de número dentro de caixa aberta** | **sim, e escala de graça** | os campos nascem de tabela com rótulo (`src/pages/mesa/grid.astro:10158` · `const CAMPOS_ATQ`). Um mecanismo só, "rótulo mais número", cobre todos |
...
(`src/pages/mesa/grid.astro:11845` · `document.querySelector('dialog[open]')`)
e desiste com o foco num campo
(`src/pages/mesa/grid.astro:11844` · `a.tagName === 'INPUT'`) · que é **exatamente** onde a voz
...
`src/pages/mesa/grid.astro:8762` · `A "PEÇA SELECIONADA" É`. Inventar esse estado pediria mexer
...
golpe exige ATACANTE E ALVO (`src/pages/mesa/grid.astro:9574` · `function folhaDaAcao`). A decisão 4
...
(`src/pages/mesa/grid.astro:10283` · `const campoAlvo = MESTRE`), porque a view da migração 27 não
