...
`src/pages/mesa/grid.astro:10393` · `O CAMPO GUARDA AS FACES, E NÃO O TOTAL`. Quem soma é a folha,
...
`src/pages/mesa/grid.astro:10577` · `const CAMPOS_ATQ`, e
`src/pages/mesa/grid.astro:10590` · `const CAMPOS_ALVO`.
...
|---|---|---|
| **campo de número dentro de caixa aberta** | **sim, e escala de graça** | os campos nascem de tabela com rótulo (`src/pages/mesa/grid.astro:10577` · `const CAMPOS_ATQ`). Um mecanismo só, "rótulo mais número", cobre todos |
...
`src/pages/mesa/grid.astro:9178` · `A "PEÇA SELECIONADA" É`. Inventar esse estado pediria mexer
...
golpe exige ATACANTE E ALVO (`src/pages/mesa/grid.astro:9990` · `function folhaDaAcao`). A decisão 4
...
(`src/pages/mesa/grid.astro:10703` · `const campoAlvo = MESTRE`), porque a view da migração 27 não
