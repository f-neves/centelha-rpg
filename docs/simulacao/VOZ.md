...
- **Desfazer não é a rede que este documento supunha.** Cobre posição e Vida
  (`src/pages/mesa/grid.astro:11715` · `async function desfazer`), não cobre Mana, a declaração do golpe, o Tick nem a
...
- **Mana não distingue dar de tirar.** É uma função só (`src/pages/mesa/grid.astro:11392` · `async function ajustarMana`), então
...
`src/pages/mesa/grid.astro:10416` · `O CAMPO GUARDA AS FACES, E NÃO O TOTAL`. Quem soma é a folha,
...
`src/pages/mesa/grid.astro:10609` · `const CAMPOS_ATQ`, e
`src/pages/mesa/grid.astro:10622` · `const CAMPOS_ALVO`.
...
|---|---|---|
| **campo de número dentro de caixa aberta** | **sim, e escala de graça** | os campos nascem de tabela com rótulo (`src/pages/mesa/grid.astro:10609` · `const CAMPOS_ATQ`). Um mecanismo só, "rótulo mais número", cobre todos |
...
(`src/pages/mesa/grid.astro:12354` · `document.querySelector('dialog[open]')`)
e desiste com o foco num campo
(`src/pages/mesa/grid.astro:12353` · `a.tagName === 'INPUT'`) · que é **exatamente** onde a voz
...
da magia (`src/lib/artes-grid-ui.ts:796` · `id="ag-busca"`). Nesses, o reconhecedor roda solto e
...
Alvos só existem como botão mais e menos (`src/lib/artes-grid-ui.ts:693` · `data-par`), então não há
...
(`src/pages/mesa/grid.astro:10735` · `const campoAlvo = MESTRE`), porque a view da migração 27 não
