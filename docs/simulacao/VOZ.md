...
- **Desfazer não é a rede que este documento supunha.** Cobre posição e Vida
  (`src/pages/mesa/grid.astro:11492` · `async function desfazer`), não cobre Mana, a declaração do golpe, o Tick nem a
...
- **Mana não distingue dar de tirar.** É uma função só (`src/pages/mesa/grid.astro:11180` · `async function ajustarMana`), então
...
`src/pages/mesa/grid.astro:10239` · `O CAMPO GUARDA AS FACES, E NÃO O TOTAL`. Quem soma é a folha,
...
`src/pages/mesa/grid.astro:10423` · `const CAMPOS_ATQ`, e
`src/pages/mesa/grid.astro:10436` · `const CAMPOS_ALVO`.
...
|---|---|---|
| **campo de número dentro de caixa aberta** | **sim, e escala de graça** | os campos nascem de tabela com rótulo (`src/pages/mesa/grid.astro:10423` · `const CAMPOS_ATQ`). Um mecanismo só, "rótulo mais número", cobre todos |
...
(`src/pages/mesa/grid.astro:12132` · `document.querySelector('dialog[open]')`)
e desiste com o foco num campo
(`src/pages/mesa/grid.astro:12131` · `a.tagName === 'INPUT'`) · que é **exatamente** onde a voz
...
`src/pages/mesa/grid.astro:9024` · `A "PEÇA SELECIONADA" É`. Inventar esse estado pediria mexer
...
golpe exige ATACANTE E ALVO (`src/pages/mesa/grid.astro:9836` · `function folhaDaAcao`). A decisão 4
...
(`src/pages/mesa/grid.astro:10549` · `const campoAlvo = MESTRE`), porque a view da migração 27 não
