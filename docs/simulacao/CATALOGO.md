...
| **a medida pedida sobre uma premissa não conferida** | mandar alguém medir X dizendo "se X, então Y" | eu conferi que **Y depende de X de verdade**, ou só supus? Em 12/09/2026 o Arquiteto mandou medir se uma fila de criaturas era aliada ou inimiga, dizendo que se fosse aliada a decisão nova resolveria o cenário. A medida saiu certa e **não era carregadora**: nenhuma das três funções de ocupação lê o campo de grupo, então o conserto era necessário de um jeito ou de outro. Quem recebe gasta a medição e, pior, pode concluir a partir dela. **A premissa da instrução precisa da mesma conferência que o resultado dela.** |
| **a causa velha que não salva a promessa nova** *(regra de CLASSIFICAÇÃO, não padrão de erro)* | classificar um achado como ESCALA porque o código que o causa é pré-existente | a rodada AFIRMA alguma cobertura que este achado falsifica? Causa antiga não basta para ESCALA quando o item da rodada nomeia por escrito o caminho que ficou descoberto: aí a causa é velha e a **promessa quebrada nasceu hoje**, e fechar como ESCALA deixaria o registro afirmando mais do que foi medido. Achada em 12/09/2026 numa discordância entre Revisora e Arquiteto sobre o mesmo achado, com a mesma medida. |
| **parar na linha que você veio ver** | achar o defeito que se procurava e registrar sem ler as vizinhas | o que eu achei é a **única** coisa errada aqui, ou só a que eu estava procurando? Em 12/09/2026 a Executora viu a linha que não marcava a posição como pendente e parou; as duas vizinhas descartavam o erro da gravação e escreviam no registro que a peça tinha andado. **Três descuidos, um ângulo cada, na mesma passagem.** Defeito raramente mora sozinho, e dois minutos nas linhas de cima e de baixo custam menos que a segunda rodada. |
| **a regra mora na escrita, e o chamador faz o que quiser com a resposta** | pôr uma conferência no ponto único de gravação e dar o invariante por fechado | a escrita RECUSAR certo é metade; a outra metade é alguém OLHAR a recusa. Um chamador que descarta o `error`, deixa o cache otimista de pé e ainda narra o sucesso no registro produz um estrago que a porta não pode impedir, porque a porta fez o trabalho dela. Ao fechar um estrangulamento, meça também **quem ignora a resposta dele**. |
| **o instrumento que filtra a própria saída** | conferir uma varredura com `git diff`/`git show` neste ambiente (travessão, coautoria, "mudou só isto?") | o `git` está passando por um hook que ENCOLHE a saída? Medido em 12/09/2026 pela Revisora e conferido pelo Arquiteto: `git show` de um commit devolveu **518** linhas pelo caminho normal e **800** por `rtk proxy git show`. A varredura roda, o `grep` acha zero, e o zero é sobre um pedaço. Para varrer, `rtk proxy git ...`, ou leia o ARQUIVO em vez do diff. |
| **a âncora que a quebra de linha separou da citação** | escrever `` `âncora` `` no fim de uma linha do documento e `` (`arquivo:NNN`) `` no começo da seguinte | o portão procura a âncora na MESMA linha da citação, e prosa de 100 colunas separa as duas sem que nada pareça errado. Aconteceu três vezes num dia só, sempre com o texto certo. Escreva âncora e citação grudadas e quebre a linha em outro lugar. |
| **a escrita solta, sem o pacote em volta dela** | `update({...})`, um objeto de colunas montado campo a campo, toda escrita achada por varredura de um campo | o que mais viaja **neste mesmo update**? Uma escrita que sobe a Vida fora de qualquer trava parece buraco; com `tick = 0`, `condicoes = []` e `ativo = true` viajando junto, ela é um **reset de cena** que passa por ali, e o gesto é outro. Achada em 16/09/2026: a Revisora mediu a escrita e não o pacote, e escreveu isso ela mesma · *"é o pacote que separa recomeçar de curar"*. Varrer por campo acha o ponto; só os vizinhos do mesmo write dizem de que gesto ele faz parte. **E ela rende no ROTEIRO DE QUEM CONFERE, e não no de quem digita** (dito pela Revisora ao ler a entrada): o gatilho é de símbolo, mas a pergunta exige LER os vizinhos, e isso não é gesto de quem está escrevendo a linha. Quem varre por `update(` acha o ponto; a pergunta "o que mais viaja neste write" tem de estar na lista de quem revisa, ou a forma não dispara. **E a segunda linha, de 16/09/2026, que nasceu do mesmo caso medido até o fim:** a pergunta não é o que mais viaja neste write, é o que mais viaja **NECESSARIAMENTE** nele. No caso que gerou a forma, os vizinhos que davam sentido ao gesto (zerar relógio, limpar condições) eram **caixas independentes marcadas por padrão**, cada uma no seu próprio `if`: **vizinho dentro de um `if` próprio é vizinho opcional**, e uma conclusão tirada do pacote vale só para o caminho em que o pacote inteiro viaja. **E a régua tem DOIS lados, e o segundo é o que responde a pergunta:** vizinho escrito no MESMO `if` é obrigatório, e é ele que diz o que a escrita faz de verdade. No mesmo caso, a Energia estava na mesma linha da Vida, sem caixa própria · e três correções seguidas escreveram "não faz mais nada" usando só o primeiro lado. Derrubar o pacote com o lado A e não perguntar o lado B deixa a frase errada de um jeito novo a cada tentativa. |
| **o cabeçalho que generaliza o que o corpo restringe** | *por gesto:* titular uma seção de decisão, ou resumir numa linha uma regra que vale só para um caso | o título repete a RESTRIÇÃO, ou só o efeito? Título é o que viaja: quem cita uma decisão de duas páginas cita a linha de cima. Em 15/09/2026 uma seção intitulada "o Impacto não atravessa o zero", cujo corpo dizia "para quem TEM A TÉCNICA", foi citada pelo cabeçalho e a rodada seguinte publicou a leitura geral em **dois arquivos de dado comprável** · o que revogaria a regra que a mesa tinha acabado de fixar e faria a Técnica de nível 4 comprar nada. E o agravante que a Revisora nomeou: a regra de precedência da casa (o JSON vence, o capítulo se corrige) aplicada sobre o dado errado **executa o erro com autoridade**. |
...
citação, `src/pages/mesa/grid.astro:4678` · `function grupoDaVez`, tem trinta e tantos caracteres), enquanto uma âncora
...
este achado: `grupoDaVez` (`grid.astro:4678`) tinha a âncora certa (`function grupoDaVez`) escrita
