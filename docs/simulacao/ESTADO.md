...
(`avancarAteParar`, `grid.astro:5778-5792`) entrega mais do que o PISO assumia:
...
| "não há lance em que o veredito não seja derivável" | há **três caminhos que devolvem `null`**: `lance.ts:144` (`if (alvo.defesaBase == null) return null`), `grid.astro:9182` (o ternário exige `soma != null && def2 != null`) e `grid.astro:9088` (`defesaBase: r?.defesa ?? null`) |
...
dos três botões (`sim.classList.toggle('primary', ...)`, `grid.astro:9294-9296`).
Sem soma ou sem Defesa, nenhum é destacado e a caixa diz o que falta
(`if (L.soma == null || L.defesa == null)`, `grid.astro:9263`).
...
`avancarAteParar` (`grid.astro:5778`), ao achar `instanteDeGolpe()`, não abre só
...
antes de parar: `for (const c of emPe) {`, `grid.astro:5788`. O item 5 ("a
...
`if (contadorDeMordidas() > mordidasAntes) return;`, `grid.astro:5800` — o cartão
