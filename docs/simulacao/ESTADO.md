...
(`avancarAteParar`, `grid.astro:5923-5937`) entrega mais do que o PISO assumia:
...
| "não há lance em que o veredito não seja derivável" | há **três caminhos que devolvem `null`**: `lance.ts:144` (`if (alvo.defesaBase == null) return null`), `grid.astro:10014` (o ternário exige `soma != null && def2 != null`) e `grid.astro:9920` (`defesaBase: r?.defesa ?? null`) |
...
(`if (L.soma == null || L.defesa == null)`, `grid.astro:10095`).
...
`if (contadorDeMordidas() > mordidasAntes) return;`, `grid.astro:5945` · o cartão
