...
| "não há lance em que o veredito não seja derivável" | há **três caminhos que devolvem `null`**: `lance.ts:144` (`if (alvo.defesaBase == null) return null`), `grid.astro:9600` (o ternário exige `soma != null && def2 != null`) e `grid.astro:9506` (`defesaBase: r?.defesa ?? null`) |
...
dos três botões (`sim.classList.toggle('primary', ...)`, `grid.astro:9712-9714`).
Sem soma ou sem Defesa, nenhum é destacado e a caixa diz o que falta
(`if (L.soma == null || L.defesa == null)`, `grid.astro:9681`).
