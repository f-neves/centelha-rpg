...
(`avancarAteParar`, `grid.astro:5993-6008`) entrega mais do que o PISO assumia:
...
| "não há lance em que o veredito não seja derivável" | há **três caminhos que devolvem `null`**: `lance.ts:144` (`if (alvo.defesaBase == null) return null`), `grid.astro:10248` (o ternário exige `soma != null && def2 != null`) e `grid.astro:10154` (`defesaBase: r?.defesa ?? null`) |
...
dos três botões (`sim.classList.toggle('primary', ...)`, `grid.astro:10360-10362`).
Sem soma ou sem Defesa, nenhum é destacado e a caixa diz o que falta
(`if (L.soma == null || L.defesa == null)`, `grid.astro:10329`).
...
`avancarAteParar` (`grid.astro:5993`), ao achar `instanteDeGolpe()`, não abre só
...
antes de parar: `for (const c of emPe) {`, `grid.astro:6003`. O item 5 ("a
...
`if (contadorDeMordidas() > mordidasAntes) return;`, `grid.astro:6016` · o cartão
