...
(`avancarAteParar`, `grid.astro:5971-5986`) entrega mais do que o PISO assumia:
...
| "não há lance em que o veredito não seja derivável" | há **três caminhos que devolvem `null`**: `lance.ts:144` (`if (alvo.defesaBase == null) return null`), `grid.astro:10218` (o ternário exige `soma != null && def2 != null`) e `grid.astro:10124` (`defesaBase: r?.defesa ?? null`) |
...
dos três botões (`sim.classList.toggle('primary', ...)`, `grid.astro:10330-10332`).
Sem soma ou sem Defesa, nenhum é destacado e a caixa diz o que falta
(`if (L.soma == null || L.defesa == null)`, `grid.astro:10299`).
...
`avancarAteParar` (`grid.astro:5971`), ao achar `instanteDeGolpe()`, não abre só
...
antes de parar: `for (const c of emPe) {`, `grid.astro:5981`. O item 5 ("a
...
`if (contadorDeMordidas() > mordidasAntes) return;`, `grid.astro:5994` · o cartão
