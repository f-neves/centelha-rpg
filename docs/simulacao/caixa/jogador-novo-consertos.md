...
| **M-29** (114) | qual é o porte de cada raça? | **ausência no dado**: `racas.json` não tem campo `porte` em nenhuma das oito, e `derivados.pv.porte` exige um porte para calcular PV. **Não há como saber o PV de um halfling.** A ficha usa Médio para todo mundo, sem dizer · **DECIDIDO** `f5b08b7`, ainda NÃO implementado: o Halfling é `pequeno` (Vigor 3 vai de 34 para 26 de PV). Gnomo e Anão seguem abertos, e o porte do PC NÃO chega ao Grid por decisão. |
