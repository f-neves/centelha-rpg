...
| **M-32** (128) | o que acontece com quem não tem a Força do Arco Composto? | o requisito existe (`forcaMin 4`, citado em `derivados.danoForca`) e **nenhuma regra diz a consequência de falhar** · **DECIDIDO** `PENDSHA`, ainda NÃO implementado: abaixo de Força 4 o Composto soma `Força×1` e parte de `+0`, que são os números do **Arco Longo**. Na Força 1 sai 1d6+1 em vez de 1d6+4. São TRÊS leitores de `forcaMult` (`ficha-engine`, `combate-resumo` que alimenta a mesa, e `lib-tempo.mjs` do espelho), e a palavra "Requer" da tabela passa a estar errada |
