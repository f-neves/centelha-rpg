...
| **M-42** (26) | "Herói" é o orçamento de 2600 ou o tier de Centelha 3? | `orcamentoHeroico` e `escalaCentelha[3].rotulo` usam a mesma palavra para coisas diferentes, e nada as desambigua · **DECIDIDO** em 16/09/2026, **FEITO** em 18/09/2026 (`2520b5d`): as chaves `orcamentoPadrao`/`orcamentoVeterano`/`orcamentoHeroico` viraram `iniciante`/`veterano`/`especialista` em `regras.json` (e em `scripts/cost-examples.mjs`, que as lia), com a mesma troca no capítulo XVIII (os três rótulos, mais o cabeçalho do exemplo do Veil). "Herói" fica só com a Centelha 3. O nome do degrau vizinho ("Grande herói" no nível 4) mudou por decisão separada no mesmo dia, para "Campeão": ver `CONTEXTO.md` |
