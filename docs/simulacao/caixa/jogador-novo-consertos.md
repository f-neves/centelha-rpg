...
| C-78 (130, 131) | a fórmula de dano de `combate.md` | a nota de `derivados.danoForca` resolve: "1 mão ×1, 2 mãos ×2 (versáteis com as duas também ×2)", e a tag "Pesada" não altera isso. A tag "Ágil" (Destreza no dano) existe em `armas.json` e não aparece na fórmula. **CORRIGE da Revisora (17/09/2026):** o item já nasceu com um engano — "Ágil usa Destreza no dano" **não existe em código nenhum** (`combate-resumo.ts` só lê `atrib` da arma para o ACERTO, nunca para o dano; o dano sempre soma Força). A frase que citei existia em `armas-e-armaduras.md` (seção Tags), mas ela também está errada, sem nenhum M-item registrado por trás. A frase que eu tinha somado a `combate.md` foi RETIRADA. **`armas-e-armaduras.md:48` corrigida junto**, mesmo veredito, sem precisar escalar de novo: a Tag "Ágil" também prometia "+1 na Defesa da Arma", e nem essa parte se sustenta (Dardos e Adaga de Arremesso têm Defesa da Arma 0; só a Adaga tem 1, e é coincidência daquela arma, não regra da tag). A tag "ágil" não é lida em código nenhum; a linha da legenda virou "só descritiva, sem efeito mecânico próprio". |
...
| **M-42** (26) | "Herói" é o orçamento de 2600 ou o tier de Centelha 3? | `orcamentoHeroico` e `escalaCentelha[3].rotulo` usam a mesma palavra para coisas diferentes, e nada as desambigua · **DECIDIDO** em 16/09/2026, **FEITO** em 18/09/2026 (`<pendente>`): as chaves `orcamentoPadrao`/`orcamentoVeterano`/`orcamentoHeroico` viraram `iniciante`/`veterano`/`especialista` em `regras.json` (e em `scripts/cost-examples.mjs`, que as lia), com a mesma troca no capítulo XVIII (os três rótulos, mais o cabeçalho do exemplo do Veil). "Herói" fica só com a Centelha 3. O nome do degrau vizinho ("Grande herói" no nível 4) mudou por decisão separada no mesmo dia, para "Campeão" — ver `CONTEXTO.md` |
