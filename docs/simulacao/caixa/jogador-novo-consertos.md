...
| C-78 (130, 131) | a fórmula de dano de `combate.md` | a nota de `derivados.danoForca` resolve: "1 mão ×1, 2 mãos ×2 (versáteis com as duas também ×2)", e a tag "Pesada" não altera isso. A tag "Ágil" (Destreza no dano) existe em `armas.json` e não aparece na fórmula. **CORRIGE da Revisora (17/09/2026):** o item já nasceu com um engano — "Ágil usa Destreza no dano" **não existe em código nenhum** (`combate-resumo.ts` só lê `atrib` da arma para o ACERTO, nunca para o dano; o dano sempre soma Força). A frase que citei existia em `armas-e-armaduras.md` (seção Tags), mas ela também está errada, sem nenhum M-item registrado por trás. A frase que eu tinha somado a `combate.md` foi RETIRADA (não substituída por outra), e o `armas-e-armaduras.md:48` continua com a mesma frase errada, sem sha meu, registrado aqui como achado pendente: alguém decide se vira mecânica nova (Destreza entra no dano de verdade) ou se o texto da Tag é que se corrige para não prometer nada. |
