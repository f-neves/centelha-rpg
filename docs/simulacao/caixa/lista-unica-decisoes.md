...
as raízes (seis desde 24/09/2026), o monte B da leitura de novata, as duas perguntas das rodadas 98 e 99, e o resto.
...
## 1 · As seis raízes (`Pendencias.md` §6, e a sexta promovida pelo humano em 24/09/2026)
...
6. **RAIZ · os dois modelos de movimento (a22 + a23 + a24)** · promovida pelo humano em 24/09/2026: as cinco acima travam conteúdo que ainda não existe; esta está em jogo AGORA, em toda mesa, em toda rodada de movimento · **o que está em jogo:** qual é a Corrida (e a Investida, que corre) · **o que o código faz hoje, lido pelo Arquiteto em 24/09/2026, e ele corrige a premissa do cruzamento:** desde `6ba575e` (28/08/2026) o Grid NÃO usa as velocidades fixas para peça com ficha ou bloco. `passoDaPeca` (`src/pages/mesa/grid.astro`) lê o passo da mesma `deslocamento()` da ficha (`src/lib/combate-resumo.ts`); o Deslocamento de Batalha é o `normal` da ficha; a Corrida e a Investida andam o `arranque` da ficha, e não a `corrida` dela ("os três primeiros Ticks de uma corrida são a aceleração"); o `andar` é 1,5 fixo para todos; o 1,5/3/6 de `MODOS_MOV` só vale para figurante sem ficha nem bloco. Antes de 28/08, 3 m por Tick para todo mundo. **Então os dois modelos são a Corrida de topo da ficha (`4 + 0,75 Des + 0,5 Atl`, a que a ficha e o capítulo mostram) contra a Corrida de arranque que o Grid anda (`2 + 0,25 For + 0,25 Atl + 0,5 Des`).** Medido pelo Arquiteto, Destreza 1 a 6, Atletismo 0 a 6, Força 1 a 6, com o arredondamento da função: a corrida de topo vai 67% a 167% além do Batalha, e o arranque, 0% a 100%. **O "50 a 67%" de `combate.md:291` e `regras.json:2477` não sai de nenhum dos dois** (as velocidades fixas dariam 100%), e de onde ele veio não foi rastreado · **qual a mesa usou até hoje:** o repositório não sabe se houve sessões nem o que se moveu nelas (o registro de produção fica atrás da RLS). O que dá para afirmar: desde 28/08, o Grid oferece por padrão a Corrida de arranque a toda peça com ficha ou bloco, e nenhum caminho dele anda a Corrida de topo sozinho; ela aparece como número mostrado (ficha, resumo, a nota do diálogo de movimento, `grid.astro:6359`), e o mestre só a anda se digitar o número no campo editável da velocidade · **opções:** (1) a Corrida de topo da ficha vale, e o Grid passa a andá-la; (2) o arranque do Grid vale como a Corrida de combate, e a ficha e o capítulo passam a mostrar o arranque (e dizer onde a topo vale, se valer); (3) as duas valem, com a regra de quando se passa do arranque ao topo, escrita · e à parte, no salto correndo: vence o JSON `saltoHorizontalCorrendo` (`regras.json:888-891`) ou vence o capítulo e o rótulo da ficha · **o que trava:** a Investida, a Corrida e o salto, na ficha e no Grid ao mesmo tempo · **a advertência do cruzamento vale:** consertar o número de um lado quebra o outro motor.
...
- a22 + a23 + a24 · **promovidos a RAIZ 6 em 24/09/2026** (seção 1).
...
- **I11** · entrou em 24/09/2026 (tinha a pergunta sem a marca) · quando a MIRA e a FORMA de uma Arte travam: na declaração (o que o código faz hoje) ou no fim do Preparo (a `§5.5` do `Arcano_revisao.md`: "no último Tick se dá a forma e a mira") · opções: travam na declaração, e quem se move durante o Preparo ESCAPA; travam no fim, e quem se move NÃO escapa · **a régua nova da conjuração (`docs/simulacao/CONJURACAO.md`) não responde**: ela trava o nível efetivo na declaração e reabre os Parâmetros no Golpe, diz que a forma não precisa estar decidida na declaração, e não diz quando ela trava nem fala da mira; os dois documentos agora apontam um para o outro · trava a implementação da régua da conjuração, e a chance de escapar de toda Arte de área.
...
**Marcados em 24/09/2026, a pedido do humano:** A11 `[DECIDIR]` (é a raiz 3, e não entra de novo na seção 4); I11 `[DECIDIR]`, que entrou na seção I acima; K28 `[DECIDIR]` (a ausência de zona de controle e de ataque de oportunidade: escolha de design ou lacuna); I12 `[FAZER]`, e não `[DECIDIR]`: a regra do arremesso existe (o alcance sai da Força de Arremesso de quem joga), e o que falta é levar esse número ao `RESUMO` do Grid. As contas desta seção são de antes das marcas e não foram refeitas.
