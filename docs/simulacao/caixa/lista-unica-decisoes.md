...
6. **RAIZ · a Corrida que o Grid anda (a22 + a23 + a24)** · promovida pelo humano em 24/09/2026: as cinco acima travam conteúdo que ainda não existe; esta está em jogo AGORA, em toda mesa, em toda rodada de movimento · **A ESCOLHA DE HOJE NUNCA FOI DECIDIDA PELO HUMANO.** Ela mora num comentário de código (`passoNoModo`, `src/pages/mesa/grid.astro`: "os três primeiros Ticks de uma corrida são a aceleração, e quase toda perseguição de combate cabe neles"), escrito por quem implementou o passo real em `6ba575e` (28/08/2026) · **o que a regra publicada diz, desde 11/06/2026 (`1418de1`):** `combate.md:298-302` e a nota de `regras.json` `derivados.deslocamento`: os 3 primeiros Ticks de uma Corrida vão na velocidade de ARRANQUE (`2 + Força ÷ 4 + Atletismo ÷ 4 + Destreza ÷ 2`), e do 4º em diante na de CORRIDA (`4 + 0,75 Destreza + 0,5 Atletismo`) · **o que o Grid faz:** anda o arranque em TODOS os Ticks de uma Corrida e de uma Investida, e nunca passa à corrida no 4º; o Deslocamento de Batalha é o `normal` da ficha, o andar é 1,5 fixo para todos, e o 1,5/3/6 de `MODOS_MOV` só vale para figurante sem ficha nem bloco (antes de 28/08, 3 m por Tick para todo mundo). Não são dois motores: é o mesmo motor, e o Grid aplica só a primeira metade de uma regra publicada · **a conta pedida pelo humano, o personagem mais fraco (Destreza 1, Atletismo 0):** Batalha 2 m por Tick; Arranque 3 (+50%) com Força 1 a 3, e 4 (+100%) com Força 4 a 6; a Corrida de topo seria 5 (+150%). Ele não fica parado. O "0%" que o Arquiteto mediu antes existe, mas em outros 8 dos 252 perfis (Destreza 1 ou 2, Força 1 ou 2, e Atletismo que sobe o Batalha), e é arredondamento: `D1 A1 F1`, `D1 A1 F2`, `D1 A2 F1`, `D1 A5 F1`, `D1 A5 F2`, `D1 A6 F1`, `D2 A0 F1`, `D2 A4 F1`. Em 111 dos 252 o arranque fica abaixo de +50%. Sem a penalidade de armadura e sem a fração de raça · **o "50 a 67%" publicado, rastreado com `git log -S`:** entrou em `0f191fe` (22/08/2026), junto com o exemplo "Kael anda 4 m por Tick e corre 6". As fórmulas daquele dia são as de hoje (as bases entraram horas antes, em `a0999da`), e o 6 do Kael é o ARRANQUE dele (Força 3, Destreza 4, Atletismo 3: Batalha 4, Arranque 6, topo 9). **Não é número de um modelo morto:** é a leitura do arranque, a mesma do Grid, escrita seis dias antes dele, com uma faixa mais estreita do que a fórmula dá (a medida acima vai de 0% a 100%). Então ele não sai por conserto sozinho: depende desta raiz · **opções:** (1) o Grid passa a seguir a regra publicada inteira (arranque nos 3 primeiros Ticks, corrida do 4º em diante), e o capítulo corrige a faixa; (2) a Corrida de combate é só o arranque, a regra publicada se reescreve para dizer isso, e a corrida de topo vira movimento fora do combate; (3) a regra publicada fica, e o Grid continua no arranque como simplificação declarada no capítulo · e à parte, no salto correndo: vence o JSON `saltoHorizontalCorrendo` (`regras.json:888-891`) ou vence o capítulo e o rótulo da ficha · **o que trava:** a Investida, a Corrida e o salto, na ficha e no Grid ao mesmo tempo, e o "50 a 67%" · **a advertência do cruzamento vale:** consertar o número de um lado quebra o outro.
