...
- **O tabuleiro abre a 100% de zoom.** `caber()` (`grid.astro:6707` · `function caber`) só roda no botão ⤢, na tela
...
- **O menu da peça mede 439px de altura**, 10 itens de 36px. Em retrato passa raspando; em
  **paisagem** (390px de altura) não cabe, e o encaixe de `grid.astro:7661` (`innerHeight - r.height - 8`)
...
- `.gr-palco` ganha `max-height: 70vh` abaixo de 1100px (`grid.astro:1074` (`max-height: 70vh`)) e `ajustarAltura()`
  (`grid.astro:4187` · `function ajustarAltura`) desliga abaixo de 1101px de propósito, porque a grade vira pilha. As duas
...
- **A mira no dedo** (`artes-grid-mesa.ts:527-532`): a pergunta certa é `pointer: coarse`, e não a
...
- **O palco toma o que sobrou**, com `100dvh` e não `vh` (a barra do navegador do celular entra e
  ...
  ramo para o modo telefone em vez de desligar; o `max-height: 70vh` de `grid.astro:1074` sai.
...
**O molde, em `grid.astro:2340-2364`, ganha um ramo para `body.grid-mob`:**
...
- **`arte-dlg`, o ajuste da arte.** O caso difícil, e o único que muda de ideia. Ele é **de
  propósito** um painel não modal (`grid.astro:6883` · `function modoAjuste`), porque o gesto principal do ajuste é arrastar
...
- **Toque longo** já abre o menu (`grid.astro:7496` · `toque longo`, 400ms com folga de 8px). Fica, e ganha
