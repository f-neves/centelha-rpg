...
- **O tabuleiro abre a 100% de zoom.** `caber()` (`grid.astro:6875` · `function caber`) só roda no botão ⤢, na tela
...
- **O menu da peça mede 439px de altura**, 10 itens de 36px. Em retrato passa raspando; em
  **paisagem** (390px de altura) não cabe, e o encaixe de `grid.astro:7835` (`innerHeight - r.height - 8`)
  devolve um topo **negativo**: o menu sai pela borda de cima e não há rolagem
  dentro dele.
- **`.rg-acs` (`grid.astro:1961`) nasce com `opacity: 0` e só acende no `:hover`.** No dedo não há
...
- `.gr-palco` ganha `max-height: 70vh` abaixo de 1100px (`grid.astro:1073` (`max-height: 70vh`)) e `ajustarAltura()`
  (`grid.astro:4232` · `function ajustarAltura`) desliga abaixo de 1101px de propósito, porque a grade vira pilha. As duas
...
- **O palco toma o que sobrou**, com `100dvh` e não `vh` (a barra do navegador do celular entra e
  ...
  ramo para o modo telefone em vez de desligar; o `max-height: 70vh` de `grid.astro:1073` sai.
...
**O molde, em `grid.astro:2337-2361`, ganha um ramo para `body.grid-mob`:**
...
- **`arte-dlg`, o ajuste da arte.** O caso difícil, e o único que muda de ideia. Ele é **de
  propósito** um painel não modal (`grid.astro:7051` · `function modoAjuste`), porque o gesto principal do ajuste é arrastar
...
- **Toque longo** já abre o menu (`grid.astro:7670` · `toque longo`, 400ms com folga de 8px). Fica, e ganha
