...
- **O tabuleiro abre a 100% de zoom.** `caber()` (`grid.astro:6678` · `function caber`) só roda no botão ⤢, na tela
...
- **O menu da peça mede 439px de altura**, 10 itens de 36px. Em retrato passa raspando; em
  **paisagem** (390px de altura) não cabe, e o encaixe de `grid.astro:7631` (`innerHeight - r.height - 8`)
...
- **`arte-dlg`, o ajuste da arte.** O caso difícil, e o único que muda de ideia. Ele é **de
  propósito** um painel não modal (`grid.astro:6854` · `function modoAjuste`), porque o gesto principal do ajuste é arrastar
...
- **Toque longo** já abre o menu (`grid.astro:7466` · `toque longo`, 400ms com folga de 8px). Fica, e ganha
