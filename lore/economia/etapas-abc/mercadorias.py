...
add("comida","alimentos","peixe-fresco","Peixe fresco","1 kg",0.5,"d","Derivado","C","Sem fonte direta; posto entre o arenque e a carne. Litoral e rios: modificador regional forte.",None,1)
...
add("comida","alimentos","racao-dia","Ração de viagem (1 dia)","1 dia",0.45,"d","Derivado","C","Pão duro, queijo e carne salgada para um dia (~0,6 kg de pão, 0,15 de queijo, 0,1 de toucinho) mais embalagem. Custa o mesmo que três refeições de estalagem pobre, e não mais.",2,1)
# ---------- Bebidas
...
add("geral","materia-prima","linho-grosso","Linho comum","jarda (~0,9 m)",3,"d","Derivado","C","Derivado da camisa de linho de Hodges (8 d em 1313). Cesta de subsistência e respeitável.",None,0.25)
...
add("roupa","roupas","vestes","Vestes oficiais ou cerimoniais","conjunto",576,"C","MtD","B","O catálogo tem 'Vestes (cerimoniais)' a 5 pp.",50,3)
...
add("geral","luz","frasco-oleo","Frasco de óleo de lamparina","frasco (~0,5 L)",1.5,"d","Derivado","C","Óleo vegetal ou de peixe, abaixo do sebo por litro. Mantém o id do catálogo.",10,0.5)
...
# ---------- Aventura e viagem
add("geral","aventura","saco-dormir","Saco de dormir (rolo de lona e lã)","unidade",dv(5.5+1,0.3),"d","Derivado","C","Uma jarda quadrada de lona (5,5 d), manta fina de enchimento (1 d) e costura. Fica acima do lençol e abaixo do cobertor de lã.",10,2.5)
...
add("geral","aventura","corda","Corda de cânhamo (15 m)","15 m (~1,7 kg)",dv(3.75*0.5,0.3),"d","Derivado","C","3,75 libras de cânhamo mais o cordoeiro. A MtD daria ~94 d, provável cabo de navio. Mantém o id do catálogo.",10,1.7)
...
add("geral","aventura","bloco-incenso","Bloco de incenso","unidade (~50 g)",1.5,"d","Derivado","C","Resina importada, ~2s por libra.",5,0.05)
...
# ---------- Escrita
add("geral","escrita","folha-papel","Folha de papel","folha",8,"C","MtD","B","Papel era raro e importado antes de 1400. Mantém o id do catálogo.",2,0.01)
...
add("geral","escrita","livro-estudo","Livro de estudo","volume",1440,"C","MtD","B","Hodges: 7 livros por ~£5 (1479). Mantém o id do catálogo.",250,2)
...
def checar_coerencia():
    ...
    testes = [
        ...
        ("lençol < saco de dormir < cobertor", p("lencol") < p("saco-dormir") < p("cobertor")),
