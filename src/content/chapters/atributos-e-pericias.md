...
|:---:|---|
| **1** | Deficiente — abaixo da média; chega a atrapalhar. |
| **2** | Comum — a média das pessoas, sem treino. |
| **3** | Acima da média — treinado, competente, notável. |
| **4** | Excepcional — entre os melhores que se vê. |
| **5** | Pico humano — dos maiores que já viveram. |

Essa régua é geral: um **3** em Força é "forte"; um **3** em Inteligência é "culto e esperto". Cada Atributo abaixo traz a sua própria leitura dos cinco degraus.
...
**Compostura** — O **controle da própria máscara**: maneirismos, pose e o quanto você esconde do que sente e pretende. É a defesa social — sustenta a **Defesa Social** (contra quem tenta te ler) e o *raio* de quanto da própria Aparência você consegue mascarar. Na maior parte do tempo trabalha de forma passiva. *Principais Habilidades:* Sociabilidade, Manha.
...
- **Furtividade** — Mover-se sem ser visto nem ouvido. *(Destreza)*
- **Prestidigitação** — Dedos rápidos: escamotear e plantar objetos, bater carteiras, truques de palco. *(Destreza)*
...
- **Oratória** — Discursar e mover plateias. *(Influência)*
...
A Especialidade é **treino concentrado dentro de uma Habilidade primária**. Ela não eleva o teto da Habilidade; torna seus resultados **mais confiáveis** quando o foco se aplica.

Cada Especialidade aponta para **um escopo único e estreito** da Habilidade-mãe — e aqui está a diferença para uma Secundária: a Secundária pode ser um campo inteiro; a Especialidade tem que ser **um recorte só** dentro da primária, nunca tão geral. *Armas de Uma Mão* pode especializar "espada longa" ou "duelo" — não "armas brancas" (amplo demais). *Furtividade* pode especializar "nas sombras" ou "em meio à multidão" — não "ser furtivo" (que já é a Habilidade inteira).

Ela vai de **0 até o nível da Habilidade** correspondente (Habilidade 3 admite no máximo Especialidade 3) e pertence **àquela Habilidade**: a Especialidade de Furtividade não ajuda em Atletismo.

Mecanicamente: para cada ponto que se aplica, role **+1d6 e descarte o menor** dado do pool, até o tanto de pontos que você tem. Em valores fixos (como a Defesa), cada ponto vale **+1**.

<div class="callout exemplo"><span class="lbl">Exemplo</span>Com base 3d6+2 e Especialidade 2, você rola 5d6+2 e descarta os dois dados mais baixos.</div>
